...
</style></head><body><main><header><div><div class="eyebrow">0xPulsePlay / TxLINE Kit / Evidence ledger</div><h1>Phase ${escape(phase)} proof</h1><p>Deterministic implementation evidence, UAT findings, and responsive visual inspection.</p></div><div class="status">GATE EVIDENCE</div></header><section><h2>Acceptance record</h2><pre>${escape(await read("README.md"))}</pre></section><section><h2>Human-simulated UAT</h2><pre>${escape(await read("UAT.md"))}</pre></section><section><h2>Screenshot matrix</h2><div class="gallery">${gallery || "<p>No screenshots captured yet.</p>"}</div></section></main></body></html>`;
